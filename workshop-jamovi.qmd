...
-   [Jamovi](https://www.jamovi.org/) ist ein Spreadsheet-Programm zur statistischen Analyse ähnlich zu SPSS
...
-   keine Statistikeinführung, sondern eigenständiges "Hands-On"
...
Importiere die Excel-Datei "data.xlsx" in Jamovi, setze Beschreibungen, Skalentypen und Stufen aller Variablen und speichere den Datensatz.
:::
...
Erstelle für die Variablen **lt_pre, lt_post, lt_followup und mot_1 - mot_5** eine Tabelle mit **deskriptiven Statistiken** (Standardeinstellung plus *Modalwert*) sowie ein **Box-Plots (inkl. Ausreisser, Violin-Plot und Daten)**.
:::
...
Die Reliabilität der Items **mot_1 - mot_5** hat sich als akzeptabel herausgestellt (Cronbach α \> 0.70). Erstelle deshalb pro "Subject" den Mittelwert über aller Motivations-Items. Nenne die neue Variable **"mot"**.
:::
...
Führe eine **z-Standardisierung** (Mittelwert: 0, Standardbweichung: 1) der Variable **"mot"** durch. Nenne die neue Variable **"mot_z"**.
:::
...
Vergleiche die beiden Gruppen in den Variablen **lt_pre, lt_post, lt_post und mot_z** mittels einem **t-Test für unabhängige Stichproben**.

Lass die **Effektstärke** als zusätzliche Statistik ausgeben und führe den **Homogenitäts und Normalverteilungs-Test** durch.
:::
...
Kleiner Seitenhieb: In SPSS lassen sich Effektstärken bei t-Tests bis heute nicht ohne Weiteres berechnen.
:::

## t-Test {visibility="uncounted"}

{{< video resources/t-test.mp4 >}}
...
Dokumentiere den Output der eben durchgeführten t-Tests für unabhängige Stichproben. Kopiere die Tabelle mit den p-Werten in einem Microsoft-Word-Dokument.
:::
...
::: columns
::: column
-   Einzelne oder alle Analysen können über via Rechtsklick \> Löschen kopiert, exportiert und entfernt werden.
...
Lass die **partielle η^2^** als Effektstärke ausgeben, führe **Posthoc-Tests der time x group Interaktion** durch und stelle die geschätzten Randmittel in einem **time x group-Diagramm** (inkl. beobachteter Werte) dar.
:::
...
-   R als Basis: "Syntax"-Modus zeigt Syntax, welche nach R kopiert und ausgeführt werden kann (auch ohne Jamovi-Oberfläche)
...
-   R-Editor innerhalb von Jamovi (Rj): Möglichkeit zu feinkörnigen Analysen und Darstellungen ohne Jamovi verlassen zu müssen
